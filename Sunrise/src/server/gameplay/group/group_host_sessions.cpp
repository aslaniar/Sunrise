...
void allocate_claimed_host_sessions() noexcept {
    ...
    for (;;) {
        ...
        // NAMED, not defaulted (our no-defaulted-AccountKey rule, FINDINGS 20.22): a group
        // session belongs to a REGION and not to a peer, so there is no per-peer context to
        // key it on. It allocates from the legacy slot's band, which makes every region's
        // host session slot-zero's regardless of which peer advertised it. Revisit when the
        // admission path stops aliasing two peers onto one session record.
