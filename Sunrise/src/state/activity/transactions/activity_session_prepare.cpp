...
/**
 * Reads the account half the published session soid carries for one calling account.
 ...
 */
[[nodiscard]] std::uint64_t session_soid_base(const AccountKey key) noexcept {
    // The session REGISTRY is process-wide (runtime::storage::g_activity); the only
    // per-account part of an allocation is the account half of its published soid, which is
    // the calling slot's own band. While provisioned slots share a rebased identity this is
    // numerically one value; when identities split it stays correct without another change.
    const AccountState account = account_snapshot(key);
...
bool prepare_session(const AccountKey key,
                     std::uint64_t& sessionId,
                     PendingAllocation& allocation) noexcept {
    sessionId = kAbsentSessionId;
    allocation = {};
    const std::uint64_t soidBase = session_soid_base(key);
...
bool prepare_session(const AccountKey key,
    ...
    AcquireSRWLockShared(&runtime::storage::g_stateLock);
    const bool ready = prepare_locked(
        runtime::storage::g_activity, selection, soidBase, sessionId, allocation);
    ReleaseSRWLockShared(&runtime::storage::g_stateLock);
    return ready;
}
